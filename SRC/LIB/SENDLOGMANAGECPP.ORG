...
{
	LogQ		= new MessageQueue<MsgLog>			( nSystemId, 1, 1, 2, 0, 10, 1000 );
	performMQ	= new MessageQueue<ProcPerformData>	( nSystemId, 1, 1, 2, 0, 11, 1000 );
	eventHstMQ	= new MessageQueue<ProcEventData>	( nSystemId, 1, 1, 2, 0, 12, 1000 );
	errorLogMQ	= new MessageQueue<ProcErrInfo>		( nSystemId, 1, 1, 2, 0, 13, 1000 );

	clsLogMgrSM	= new LogManageSM					( nSystemId, 1, 1, 2, 0 , 0 );
...
{
	clsPerForm->put_strDateTime(nFlag);
